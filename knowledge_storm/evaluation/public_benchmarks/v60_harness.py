...
def run_context_profile_benchmark(dataset, reader, output_dir, profiles=(128_000, 256_000, 512_000)):
    ...
    report = {
        "benchmark": "context-profile-pareto-v60",
...
def run_longmemeval_end_to_end(
    ...
    for raw in _iter_json_array(dataset_path, limit=limit):
        ...
        evidence_by_mode = {
            ...
            "v56_memory": _v56_retrieve(case, output_dir / "memory-index", embedding_provider, top_k),
        }
        for mode, evidence_sessions in evidence_by_mode.items():
            ...
            rows.append(row)
    modes = {}
    for mode in ("recent", "fts_session", "v56_memory"):
    ...
    report = {
        "benchmark": "longmemeval-s-end-to-end-v60",
...
def _v56_retrieve(case, root, embedding_provider, top_k):
